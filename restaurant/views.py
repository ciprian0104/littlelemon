...
def menu(request):
    ...
    return render(request, 'menu.html', context={'menu_items': json.loads(response.text)})
...
def menu_item(request, pk):
    ...
    headers = get_auth_header()
    response = requests.get(url, headers=headers)
    return render(request, 'menu_item.html', context={'menu_item': json.loads(response.text)})
...
def bookings(request):
    ...
    headers = get_auth_header()
    response = requests.get(url, headers=headers)
    return render(request, 'bookings.html', context={'bookings': response.text})